...
class eap(packet_base):
    ...
    def __str__(self):
        ...
        if hasattr(self, 'type'):
            s += ' type=%s ; data=%s' % (eap.type_names[self.type],self.data)
        return s + "]"
    ...
    def parse(self, raw):
        ...
        self.payload_len = self.length - self.MIN_LEN
        
        if self.payload_len==1:
            (self.type) = struct.unpack('!B', raw[self.MIN_LEN:self.MIN_LEN+1])
            self.data=""
            
        elif (self.payload_len>1):
            (self.type,) = struct.unpack('!B', raw[self.MIN_LEN:self.MIN_LEN+1])
            #(self.data,) = struct.unpack('%is' %(self.payload_len-1), raw[self.MIN_LEN+1:self.length])
            self.data=raw[self.MIN_LEN+1:self.length]
            
        self.parsed = True


    def hdr(self, payload=None):
        len_data = len(self.data)
        self.length = self.MIN_LEN + len_data + 1
        return struct.pack('!BBHB%is' %(len_data), self.code, self.id, self.length, self.type, self.data)
