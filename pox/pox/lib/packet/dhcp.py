...
class dhcp(packet_base):
    ...
    def appendOption(self, code, bytes):
        unpack = _dhcp_option_unpackers.get(code, DHCPRawOption.unpack)
        self.options[code] = unpack(bytes,code)
...
@dhcp_option_def(dhcp.RELAY)
class DHCPRelay(DHCPRawOption):
    pass

"""
@dhcp_option_def(dhcp.RELAY)
class DHCPRelay(DHCPRawOption):
    def __init__(self, suboptions=[]):
        DHCPRawOption.__init__(self)
        self.suboptions=suboptions
        self.data=""
    
    def setOptions(self, suboption_code, data):
        size = len(data)
        b=struct.pack('!ii%is' %(size), suboption_code, size, data)
        self.suboptions.append(b)
        
    def packSubOptions(self):
        self.data=""
    ...
    def pack(self):
        self.packSubOptions()
        DHCPRawOption.pack(self)
        
    def unpackSubOptions(self, raw):
        size_raw = len(raw)
        position = 0    
        while True:
            code, length = struct.unpack('!BB', raw[position:position+2])
            self.suboptions.append(raw[position:position+length])
            position+=length
            
            if position>=size_raw:
                break
            
    @classmethod
    def unpack(cls, data, code=None, bad=False):
        self = cls()
        self.unpackSubOptions(data)
        return super(DHCPRelay, cls).unpack(data, code=code, bad=bad)
    
    def unpack(cls, data, code=None, bad=False):
        unpackSubOptions(data)
        DHCPRawOption.unpack(self)
    """    

@dhcp_option_def(dhcp.USER_NAME)
class DHCPUsername (DHCPRawOption):
  pass
